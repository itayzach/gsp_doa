...
B = 1;                % amplitude [V]
delta = 100e-3;        % uniform spacing between sensors [m]
c = 340;              % speed of sound [m/s]

theta = 70.3;                    % [degrees]
tau = delta*cos(theta*pi/180)/c; % [sec]

d = exp(-1j*w0*(m-1)*tau); % steering vector

%% Signals
% after a bandpass filter around f0 (filter out the negative spectrum)
x1_bp_noiseless = B*exp(1j*w0*n);
SNR = inf;
sigma = B/10^(SNR/20);
...
est_theta_vec = 0:180;
% est_theta_vec = [30 70 120];
piquancy = zeros(size(est_theta_vec));

for th_idx = 1:length(est_theta_vec)
    %% Space-domain adjacency matrix
    est_theta = est_theta_vec(th_idx);
    est_tau = delta*cos(est_theta*pi/180)/c;    % [sec]

    a1_r = 0.5*[0 exp(1j*w0*est_tau) zeros(1,M-3) exp(1j*w0*(M-1)*est_tau)];
    ...
    a2_r = 0.5*[0 exp(-1j*w0) zeros(1,N-3) exp(-1j*w0*(N-1))];
...
%     figure; stem(abs(x_hat))
    ...
    piquancy(th_idx) = 1/sqrt(sum(abs(x_hat_).^2));
end
piquancy = piquancy/max(piquancy);
figure; plot(est_theta_vec, piquancy);
